...
``` r
rnp_teste_ks(x, y = NULL, dist = "pnorm", ..., digits = 4L)
```
...
- y:
  ...
  Vetor numerico (duas amostras) ou `NULL`.

- dist:

  String com a distribuicao acumulada (ex.: `"pnorm"`) quando `y` for
  `NULL`.
...
#>         <dbl>   <dbl> <chr>      
#> 1      0.0947   0.332 uma amostra
rnp_teste_ks(rnorm(50), rnorm(50, 1))
#> # A tibble: 1 × 3
#>   estatistica p_valor metodo       
#>         <dbl>   <dbl> <chr>        
#> 1        0.54       0 duas amostras
```
