...
Testa se a mediana das diferencas (pareado) ou de `x - mu` e zero,
contando os sinais e usando a distribuicao binomial.

## Usage

``` r
rnp_teste_sinais(x, y = NULL, mu = 0, digits = 4L)
```
...
- y:
  ...
  Vetor numerico (pareado) ou `NULL`.

- mu:

  Valor de referencia (uma amostra).
...
tibble com `n_positivos`, `n_negativos`, `p_valor`.
...
#> # A tibble: 1 × 3
#>   n_positivos n_negativos p_valor
#>         <int>       <int>   <dbl>
#> 1           3           1   0.625
```
