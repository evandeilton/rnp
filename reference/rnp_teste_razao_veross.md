...
Compara dois modelos aninhados (`lm`/`glm`) pela estatistica 2
(logL_completo - logL_reduzido) ~ qui-quadrado.

## Usage

``` r
rnp_teste_razao_veross(modelo_completo, modelo_reduzido, digits = 4L)
```

## Arguments

- modelo_completo, modelo_reduzido:

  Objetos `lm`/`glm` aninhados.
