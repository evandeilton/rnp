...
``` r
rnp_ts_adf(x, lag = NULL, tipo = c("constante", "tendencia"), digits = 4L)
```
...
- lag:
  ...
  Numero de defasagens aumentadas. Default automatico.

- tipo:

  String: `"constante"` (default) ou `"tendencia"`.
...
tibble com `estatistica`, `lag`, `valor_critico_5`, `p_valor_aprox`,
`estacionaria`.
